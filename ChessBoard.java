...
public class ChessBoard {
	...
	// show current game state, for viewing game in terminal --> IF TIME, CAN MAKE
	// THIS BIGGER!
	...
	public void createPieces() {
		...
		System.out.println("Enter \"0\" for normal chess game, \"1\" for rearranged game!"); // ask user for game choice
																								// --> CHANGE THIS
																								// NAME!!
		...
		if (chosenGameType == '0') { // normal chess game, add each piece normally
		...
		} else { // NEW NAME INSERTED HERE game, add pawns, king normally, prompt players for
					// back lines
			for (int i = 0; i < 8; i++) { // 8 pawns/player
			...
			String choice;
			boolean flag = true; // flag to stop players from being able to place one piece on top of another
									// piece
									// also keeps track of turns: white = true, black = false
			// add queens
			populateBoard(); // populateBoard and currentGameState called to update text based game in
								// terminal
			currentGameState();
			while (flag) {
				...
				if ((!pieceOnSpace(7, inputToCol(choice))) && (inputToRow(choice) == 7)) { // if space is empty
					...
					flag = false;
				} else { // if piece already there / input row incorrect, output error message, loop back
							// around
					System.out.println(
							"Error: there is already a piece located in this space / the input row is not correct. Try again!");
				}
			}
			while (!flag) {
