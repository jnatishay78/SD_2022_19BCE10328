...
public class ChessMove {
	...
	public ArrayList<ArrayList<Integer>> legalPieceMoves(Piece piece, boolean checkMate) {
		...
		ArrayList<ArrayList<Integer>> legalMoves = new ArrayList<ArrayList<Integer>>(); // list to be returned with
																						// legal move set
	...
	private ArrayList<ArrayList<Integer>> possKingMoves(Piece piece, boolean checkMate) {
		...
		// NOTE: DOES THERE EVEN HAVE TO BE AN ONBOARD CHECK?? --> WOULD NOT BE TOO HARD
		// TO TAKE AWAY, BUT IS NOT TOO CRAZY

		// possible horizontal / vertical / diagonal movements
		// space to the right
		if (onBoardCheck(r, c + 1) && !pieceOnSpace(r, c + 1, color)) { // on board and not a piece that is same color
																		// in space, add space
			if (checkMate) { // capture king
				row.add(r);
				col.add(c + 1);
		...
		// space above
		if (onBoardCheck(r - 1, c) && !pieceOnSpace(r - 1, c, color)) { // on board and not a piece that is same color
																		// in space, add space
			if (checkMate) { // capture king
				row.add(r - 1);
				col.add(c);
		...
		if (onBoardCheck(r - 1, c + 1) && !pieceOnSpace(r - 1, c + 1, color)) { // on board and not a piece that is same
																				// color in space, add space
			if (checkMate) { // capture king
				row.add(r - 1);
				col.add(c + 1);
			} else { // not checkmate so make sure piece is not enemy king
	...
	private ArrayList<ArrayList<Integer>> possQueenMoves(Piece piece, boolean checkMate) {
		...
		row = new ArrayList<Integer>();
		col = new ArrayList<Integer>();
		ArrayList<ArrayList<Integer>> rowAndCol = new ArrayList<ArrayList<Integer>>(); // combination array to be
																						// returned
		// queen is a combination of bishop and rook movements. So: add both to row and
		// col
	...
	// check for check! --> so if the king of the current player (current color) is
	// in the list for any of the opponent's possEnemyTargets then they are in check
	// and the flag should be thrown to show so
	public boolean checkCheck(int color) {
		...
		for (Piece p : pieces) { // iterate over all pieces
			...
			while (enemyIter.hasNext()) { // while iterator has more values
				...
				if (nextEnemy.getType() == 1 && (p.getColor() != color)) { // if the next available target for that
																			// piece is a king and the opposite color
